...
const apiChanges = {
  subjects: '2021-10-15T12:30:00.158Z',
};
...
const useEndpoint = (endpoint) => {
  ...
  useEffect(() => {
    const fetchEndpoint = async (endpoint) => {
      ...
      // get cache data
      try {
        let cache = await localforage.getItem(endpoint);

        if (cache?.lastUpdated < apiChanges[endpoint]) {
          await localforage.removeItem(endpoint);
          cache = null;
        }
